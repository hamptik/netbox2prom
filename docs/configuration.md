...
2. If the parent device has the monitoring tag (is in the fetched set), the IP is **enriched** with the device's full attributes (vendor, role, `criticality`, `os_type`, etc.).
3. The enriched IP-target is processed through the **same `groups`** as devices — conditions and labels work identically.
4. The tagged IP is always used as the ICMP target, regardless of the group's `target_field`.
5. **Global deduplication**: if a tagged IP already appears as a device target (from any group), it is skipped entirely.
6. Tagged IPs whose parent device does **not** have the monitoring tag are skipped (cannot be enriched).

Example: a server `srv-01` (with monitoring tag, `main_ip=10.0.0.1`) has two additional interface IPs `10.0.0.5` and `10.0.0.6`, both tagged with `monitoring`. All three IPs end up in the ICMP targets file — `10.0.0.1` from the device, `10.0.0.5` and `10.0.0.6` from the tagged IP addresses.
